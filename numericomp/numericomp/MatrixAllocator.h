...
{
public:
	MatrixAllocator(const size_t& rows, const size_t& columns)
		:nRows(rows), nColumns(columns), data(rows*columns, NAN)
	{

	}

	MatrixAllocator(const size_t& rows, const size_t& columns, const T& initValue)
...
template<class T> MatrixAllocator<T> operator*(MatrixAllocator<T>& lhs, MatrixAllocator<T>& rhs) {
	...
	MatrixAllocator<T> result(lhs.Rows(), rhs.Columns(), 0);
